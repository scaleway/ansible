...
- name: Create a application
  quantumsheep.scaleway.scaleway_iam_application:
...
def delete(module: AnsibleModule, client: "Client") -> None:
    ...
    if id is not None:
        resource = api.get_application(
            application_id=id, region=module.params["region"]
        )
    elif name is not None:
        resources = api.list_applications_all(name=name, region=module.params["region"])
        if len(resources) == 0:
    ...
    api.delete_application(application_id=resource.id, region=module.params["region"])
