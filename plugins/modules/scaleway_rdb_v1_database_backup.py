...
DOCUMENTATION = r"""
---
module: scaleway_rdb_v1_database_backup
short_description: Manage Scaleway rdb_v1's database_backup
description:
    - This module can be used to manage Scaleway rdb_v1's database_backup.
version_added: "2.1.0"
author:
...
- name: Create a database_backup
  scaleway.scaleway.scaleway_rdb_v1_database_backup:
...
def create(module: AnsibleModule, client: "Client") -> None:
    ...
    resource_id = module.params.pop("database_backup_id", None)
    if id is not None:
        resource = api.get_database_backup(database_backup_id=resource_id)
    ...
    resource = api.create_database_backup(**not_none_params)
...
def delete(module: AnsibleModule, client: "Client") -> None:
    ...
    if resource_id is not None:
        resource = api.get_database_backup(
            database_backup_id=resource_id,
    ...
    api.delete_database_backup(
        database_backup_id=resource.id,
        region=resource.region,
    )

    module.exit_json(
        changed=True,
        msg=f"rdb_v1's database_backup {resource.name} ({resource.id}) deleted",
    )
